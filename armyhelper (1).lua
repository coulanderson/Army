...
script_version("29.07.2024")


local dlstatus = require('moonloader').download_satus
...
local update_url = 'https://raw.githubusercontent.com/coulanderson/Army/main/update.ini'
...
function main()
    ...
    downloadUrlToFile(update_url, update_path, function (id, status)
        ...
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            uIni = inicfg.load(nil, update_path)
            if tonumber(uIni.info.vers) > script_vers then
                sampAddChatMessage('Доступно обновление. Версия:'..uIni.info.vers_text)
                update_state = true
            end
